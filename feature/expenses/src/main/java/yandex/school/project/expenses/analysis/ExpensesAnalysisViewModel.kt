...
class ExpensesAnalysisViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow<Result<ExpensesAnalysisState>>(Result.Loading)
    val uiState: StateFlow<Result<ExpensesAnalysisState>> = _uiState
    ...
    fun setPeriod(start: LocalDate, end: LocalDate) {
        _uiState.update {
            val current = (it as? Result.Success)?.data ?: ExpensesAnalysisState()
    ...
    private fun loadAnalysis() {
        val state = (uiState.value as? Result.Success)?.data ?: ExpensesAnalysisState()
